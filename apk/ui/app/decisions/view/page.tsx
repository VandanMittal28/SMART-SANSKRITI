...
export default function TripDecisionPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const local = id ? getTripDecision(id) : null
      if (local) {
        applyDecision(local)
        setMode("owner")
        if (local.shareToken) {
      ...
      // No local copy (either a fresh device, or someone joining by code) —
      // a share code alone is enough to look the decision up remotely.
  ...
  // Each install of the bundled app serves its own local WebView origin, so a
  // clickable https:// link can't be opened on a companion's phone. Share a
  // plain code instead — they paste it into "Have a code?" on their own copy
  // of the app, which looks the decision up directly from Supabase.
  const shareCode = decision.shareToken ?? null
                ...
                {!decision.shareToken ? (
                  <>
                    <p style={{ color: "#C4A882", fontSize: 13, marginBottom: 14, lineHeight: 1.6 }}>
                      Generate a code to send a companion (text, WhatsApp, out loud — however).
                      They enter it under &quot;Have a code?&quot; on their own copy of the app to see this
                      exact drift indicator and respond Agree / Needs discussion, without being
                      able to edit the plan themselves.
                    </p>
                    <button
                      onClick={handleShare}
                      disabled={shareBusy}
                    ...
                    >
                      {shareBusy ? "Sharing…" : "🔑 Generate share code"}
                    </button>
                    {shareError && <p style={{ color: "#E08A8A", fontSize: 12, marginTop: 10 }}>{shareError}</p>}
                  </>
                ) : (
                  <>
                    <p style={{ color: "#C4A882", fontSize: 12, marginBottom: 8 }}>Share this code:</p>
                    <div style={{
                      background: "rgba(0,0,0,0.25)", borderRadius: 8, padding: "8px 12px",
                      color: "#F5E6D3", fontSize: 16, fontWeight: 700, letterSpacing: "0.05em",
