...
export default function ExplorePage() {
  ...
  const arNav = useARNavigation({
    zone: { lat: zone.lat, lng: zone.lng, radius: zone.radius },
  })
  ...
  useEffect(() => {
    ...
    if (!stored) return
    const isCustom = stored === DEMO_MONUMENT_ID
    if (!isCustom && !MONUMENT_ZONES[stored]) return
    setExploreMonumentId(stored)
    if (isCustom) {
      const custom = getCustomMonument(DEMO_MONUMENT_ID)
      const firstZone = custom?.zones[0]
      setUserPos(firstZone ? { lat: firstZone.lat, lng: firstZone.lng } : EXPLORE_USER_START['taj-mahal'])
      return
    }
  ...
  // ── LIVE AR CAMERA JOURNEY ──────────────────────────────
  const enterLiveAR = useCallback(() => {
    setArViewActive(true)
    void arNav.requestPermissions()
  }, [arNav])

  const exitLiveAR = useCallback(() => {
    arNav.stop()
    setArViewActive(false)
  }, [arNav])

  const handleArArrive = useCallback(() => {
    void handleArrival()
    arNav.stop()
    setArViewActive(false)
  }, [arNav, handleArrival])
