...
interface ExploreARHudProps {
  ...
  /** Optional live-AR entry/exit affordance. Omit to keep this component's
   *  older contract (demo/fallback only, no camera activation control). */
  onEnterLiveAR?: () => void
  onExitLiveAR?: () => void
}
...
export function ExploreARHud({
  ...
  const showLiveCamera = state.mode === 'live' && Boolean(cameraStream)
    ...
    <main className="fixed inset-0 z-[55] mx-auto h-[100dvh] w-full max-w-[420px] overflow-hidden bg-[#050814] text-[#f8ecd2] shadow-[0_0_80px_rgba(0,0,0,.65)]">
      ...
      <section className="relative h-[75dvh] min-h-0 overflow-hidden bg-[#101827]">
        ...
        <div className="absolute left-4 top-4 flex items-center gap-2">
          ...
          {onEnterLiveAR && state.mode !== 'live' && (
            <button
              className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-[#efc566]/35 bg-black/45 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-[#ffe7b0]"
              onClick={onEnterLiveAR}
              type="button"
            >
              <Camera className="size-3" /> Try live camera
            </button>
          )}
          {onExitLiveAR && state.mode === 'live' && (
            <button
              className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-black/45 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-[#f6ddb0]"
              onClick={onExitLiveAR}
              type="button"
            >
              Exit live
            </button>
