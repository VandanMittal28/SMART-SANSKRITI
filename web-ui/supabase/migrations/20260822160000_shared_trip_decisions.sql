...
-- a baseline, they can persist it here and get a link a travel companion
-- can open to see the same drift indicator and react Agree / Needs
-- discussion, without needing to be a co-editor.
...
-- A companion reaches a decision via its share link, before they hold any
...
-- matching an unguessable token; it never lists or searches by anything else.
create or replace function public.get_trip_decision_by_token(p_token uuid)
...
revoke all on function public.get_trip_decision_by_token(uuid) from public;
grant execute on function public.get_trip_decision_by_token(uuid) to authenticated;
